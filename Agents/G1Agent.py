...
class G1():
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.NewStepY = 0
        self.NewStepTheta = 0
        self.SwingStepZ = 0.05

        self.WaistX_DOF, self.WaistY_DOF, self.WaistZ_DOF = 14, 15, 13
        
        self.RShX_DOF,   self.RShY_DOF,   self.RShZ_DOF   = 29, 30, 31
        self.REB_DOF                                      = 32
        self.LShX_DOF,   self.LShY_DOF,   self.LShZ_DOF   = 21, 22, 23
        self.LEB_DOF                                      = 24

        self.RHipX_DOF,  self.RHipY_DOF,   self.RHipZ_DOF   = 7, 8, 9
        self.RKneeX_DOF, self.RAnkleX_DOF, self.RAnkleY_DOF = 10, 11, 12
        
        self.LHipX_DOF,  self.LHipY_DOF,   self.LHipZ_DOF   = 1, 2, 3
        self.LKneeX_DOF, self.LAnkleX_DOF, self.LAnkleY_DOF = 4, 5, 6

        self.JointIDOrder = [self.WaistX_DOF,self.WaistY_DOF,self.WaistZ_DOF, #0-2
                            self.RShX_DOF,self.RShY_DOF,self.RShZ_DOF,self.REB_DOF, #3-6
                            self.LShX_DOF,self.LShY_DOF,self.LShZ_DOF,self.LEB_DOF, #7-10
        ...
        for j in range(numJoints):
            p.setJointMotorControl2( bodyIndex = self.robot_id, jointIndex = j, controlMode = p.POSITION_CONTROL, targetPosition = 0 )
        ...
        self.posAndori = p.getBasePositionAndOrientation(self.robot_id)
        ...
        if(self.CONFIG["clientMode"]=="GUI"):
            ...
            p.setDebugObjectColor(self.robot_id,15,objectDebugColorRGB=[1,0,0])
    ...
    def load_map(self,mapX):
        ...
        self.robot_id = p.loadURDF(__cwd__ +"/models/g1_description/g1_29dof_lock_waist_rev_1_0.urdf",[0,0,0.8])
    ...
    def getFeetCoP(self):
        ...
        #Contact points
        lCP = p.getContactPoints(bodyA=self.robot_id, bodyB=self.FloorId, linkIndexA=54) #LAnkle
        rCP = p.getContactPoints(bodyA=self.robot_id, bodyB=self.FloorId, linkIndexA=40) #RAnkle
        ...
        if lCP:
            ...
            if lCoPforce:
                ...
                lfwpos, lfwori = p.getLinkState(bodyUniqueId=self.robot_id, linkIndex=54)[0:2]
        ...
        if rCP:
            ...
            if rCoPforce:
                ...
                #Transform to local coordinates
                rfwpos, rfwori = p.getLinkState(bodyUniqueId=self.robot_id, linkIndex=40)[0:2]
    ...
    def motorTest(self,jointID,pos):
        p.setJointMotorControl2(bodyIndex=self.robot_id, jointIndex=self.JointIDOrder[jointID], controlMode=p.POSITION_CONTROL,
    ...
    def step(self, ppo_action):
        ...
        if(self.CONFIG["clientMode"]=="GUI"):
            ...
            if val!=self.param_apply_val:
                ...
                p.applyExternalForce(self.robot_id, -1, fvec, fpos, p.WORLD_FRAME)
        ...
        if (abs(self.gtime-self.twalk0) > (2*self.StepTime)-(self.SamplingTime/2)):
            ...
            vL,vA =  p.getBaseVelocity(self.robot_id)

            self.PD_Wx = (self.action[0] * 0.3) + 0*self.PDControl(-np.pi/10-self.ori[1], vA[1], 1, -0.05, [0.3,-0.3])
            self.PD_Wy = (self.action[1] * 0.05) + 0*self.PDControl(-self.ori[0], vA[0], 0.5, 0.0, [0.1,-0.1])

            self.PD_HLx = (self.action[2] * 0.25) + 1*self.PDControl(-np.pi/10-self.ori[1], vA[1], 0.5, -0.025, [0.25,-0.25])
            self.PD_HRx = (self.action[3] * 0.25) + 1*self.PDControl(-np.pi/10-self.ori[1], vA[1], 0.5, -0.025, [0.25,-0.25])

            self.PD_HLy = (self.action[4] * 0.25) + -1.*self.PDControl(0.0-self.ori[0], vA[0], 0.65, -0.095, [0.2,-0.2])
            self.PD_HRy = (self.action[5] * 0.25) + -1.*self.PDControl(0.0-self.ori[0], vA[0], 0.65, -0.095, [0.2,-0.2])
            # print( f" s_z{self.SwingStepZ :3.3f}\t L {self.PD_HLy:3.3f} \t R {self.PD_HRy:3.3f} ")

            self.PD_ALx = (self.action[6] * 0.2) + -0*self.PD_HLx
            self.PD_ARx = (self.action[7] * 0.2) + -0*self.PD_HRx

            self.PD_ALy = (self.action[8] * 0.2) + -0.*self.PD_HLy
            self.PD_ARy = (self.action[9] * 0.2) + -0.*self.PD_HRy
            ...
            self.PD_HLy = self.SAT(self.PD_HLy, 0.15, -0.15)
            self.PD_HRy = self.SAT(self.PD_HRy, 0.15, -0.15)
            ...
            self.PD_HLy = self.SAT(self.PD_HLy, 0.2, -0.)
            self.PD_HRy = self.SAT(self.PD_HRy, -0., -0.2)
        ...
        for j in range(len(self.JointIDOrder)):
            if(j in range(3,11)): f=500
            else: f=500
            p.setJointMotorControl2(bodyIndex=self.robot_id, jointIndex=self.JointIDOrder[j], controlMode=p.POSITION_CONTROL,
        ...
        if self.scenario == 8:
            terminal = terminal or (pos[2] < 0.20 or
                bool(p.getContactPoints(bodyA=self.robot_id, bodyB=self.FloorId, linkIndexA=15)) or
                bool(p.getContactPoints(bodyA=self.robot_id, bodyB=self.FloorId, linkIndexA=30)))
    ...
    def observe(self):
        ...
        for i in range(len(self.JointIDOrder)):
            posi, vel, _, torque = p.getJointState(self.robot_id,self.JointIDOrder[i]) #pos, vel, reaction force, applied torque
            ob.append(posi)    #pos
            ob.append(vel)     #vel
        ...
        linSpeedWorld,angSpeedWorld = p.getBaseVelocity(bodyUniqueId=self.robot_id)
        ...
        po, _ = p.getBasePositionAndOrientation(self.robot_id)
        ...
        if not self.ignoreLog:
            ...
            if not hasattr(self, 'logNN'):
                ...
                self.logNN = "logs\log_NN_input__" + "__scenario_" + str(self.scenario) + datetime.now().strftime("__%d.%m.%Y_%H.%M.%S") + ".csv"
    ...
    def world_to_local_transform(self,pos,ori=[0,0,0,1], framePos=None):
        ...
        """        
        b_pos, b_ori = p.getBasePositionAndOrientation(self.robot_id)
    ...
    def world_to_local_rotate(self,pos,ori=[0,0,0,1]):
        ...
        :return: local coordinates
        """        
        _, b_ori = p.getBasePositionAndOrientation(self.robot_id)
    ...
    def apply_walk_command(self):
        ...
        self.NewStepX +=     max( min( self.NewStepX_raw - self.NewStepX, 0.025) , -0.025)
        ...
        self.NewCommand = 1
        print(f"new command:{self.NewStepX:3.3f}")
    ...
    def reset(self):
        ...
        self.NewStepX_raw = 0.0
        self.NewStepY_raw = 0.0
        self.NewStepTheta_raw = 0.0
